...
class ParentExamsCubit extends Cubit<ParentExamsStates> {
  ParentExamsCubit() : super(ParentExamsInitialState());

  static ParentExamsCubit get(context) => BlocProvider.of(context);
  ...
  void changeTypeDropDownButton(String newValue)
  {
    ...
    emit(ParentExamsTypeDropDownButtonState());
  }
  void changeYearDropDownButton(String newValue)
  {
    dropDownValueYear = newValue;
    emit(ParentExamsYearDropDownButtonState());
  }


  ExamsModel? examsModel;
  List<dynamic>? subjects;
  void getStudentMarksParent(type,year,id)
  {
    emit(ParentExamsLoadingState());
    DioHelper.postData(
        url: '${PARENTEXAMS}/${id}',
        data:{
          'type_id':type,
      ...
      emit(ParentExamsSuccessState());
    }).catchError((error){
      print(error.toString());
      emit(ParentExamsErrorState(error.toString()));
    });
  }
